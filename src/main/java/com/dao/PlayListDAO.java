...
import com.operation.AudioPlayer;
import com.operation.JukeOperation;
...
public class PlayListDAO {
    ...
    public void addingSongsToPlayList() throws SQLException, ClassNotFoundException, UnsupportedAudioFileException, LineUnavailableException, IOException {
        ...
        String sqlUpdate = "INSERT INTO playlistsong(playlistId,songId) values(?,?);";
        ...
        do {
            switch (choice) {
                ...
                case (2):
                    ...
                    System.out.format("%-10s %-30s %-30s %-30s \n", "SongID", "SongName", "Duration", "Artist");
                    System.out.println("-----------------------------------------------------------------------------------------");
                    for (Songs songs : playList) {
                        System.out.format("%-10s %-30s %-30s %-30s \n", songs.getSongID(), songs.getSongName(), songs.getDuration(), songs.getArtistName());
                    }
                    System.out.println("-----------------------------------------------------------------------------------------");
                    ...
                    switch (select) {
                        /*case (1):
                            audioPlayer.PlaySong(playList.toString());
                            break;*/
                        case (2):
    ...
    public List<Songs> exsitingPlaylist() throws SQLException, ClassNotFoundException {
        ...
        int playListID = scanner.nextInt();
        String sql = "Select SongId, SongName, Duration, artistname from songs where SongId IN (Select SongId from playlistsong where playlistId = ?);";
        PreparedStatement preparedStatement = connection.prepareStatement(sql);
        preparedStatement.setInt(1,playListID);

        ResultSet resultSet1 = preparedStatement.executeQuery();
        //String sql3 = "Select * from songs where SongId = (Select songId from playlistsong where playlistId=?);";
        /*while (resultSet1.next()) {
            ...
            while (resultSet1.next()) {
                /*playListSongs.add(resultSet1.getInt("SongId"), resultSet1.getString("SongName"), resultSet1.getDouble("Duration"), resultSet1.getString("artistname"));
            for (Songs playListSongs1: playListSongs){
                System.out.println(playListSongs1);
            }*/
                int id = resultSet1.getInt("SongId");
                String name = resultSet1.getString("SongName");
                double duration = resultSet1.getDouble("Duration");
                String artist = resultSet1.getString("artistname");
                System.out.println(id+"  "+name+"  "+duration+"  "+artist);
            }
